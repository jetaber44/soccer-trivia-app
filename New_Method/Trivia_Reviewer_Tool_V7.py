# Trivia_Reviewer_Tool_V2.py
# Enhanced GUI tool to review soccer trivia questions with improved usability
...
class TriviaReviewer:
    ...
    def setup_gui(self):
        self.master.title("Enhanced Trivia Reviewer Tool V2")
    ...
    def clean_json_content(self, raw_text):
        """Enhanced JSON cleaning to handle common formatting issues"""
        # Remove ... placeholders
        ...
        # Fix missing commas between objects (basic case)
        cleaned = re.sub(r'}\s*{', '},{', cleaned)
        
        return cleaned

    def load_questions(self, file_path):
        """Load and clean questions from file"""
        try:
    ...
    def display_json_question(self, q):
        ...
        display_text += f"\"category\": \"{q['category']}\",\n"
        subcategories_str = ', '.join([f'"{sub}"' for sub in q['subcategories']])
    ...
    def display_preview_question(self, q):
        ...
        preview_text += f"CATEGORY: {q['category']}\n"
        preview_text += f"SUBCATEGORIES: {', '.join(q['subcategories'])}\n"
    ...
    def save_to_code_file(self, question, code):
        ...
        try:
            ...
            messagebox.showerror("Save Error", f"Error saving to code file: {str(e)}")
    ...
    def remove_from_code_file(self, question, code):
        ...
        try:
            ...
            messagebox.showerror("Remove Error", f"Error removing from code file: {str(e)}")
    ...
    def edit_current_question(self):
        ...
        category_var = tk.StringVar(value=q['category'])
        ...
        difficulty_combo = ttk.Combobox(edit_window, textvariable=difficulty_var, 
                                      values=["Easy", "Medium", "Hard"], width=20)
